...
- `docker compose up -d` - Run the application with Docker (includes PostgreSQL 17, Redis, Quartz app, and SearXNG)
- `docker compose down` - Stop all containers
- `docker compose down -v` - Stop all containers and remove volumes (deletes database data)
- `docker pull ghcr.io/quartz-ai/quartz:latest` - Pull prebuilt Docker image
...
- Quartz Cloud deployments block `ENABLE_AUTH=false` automatically
...
- Guards against `QUARTZ_CLOUD_DEPLOYMENT=true`
